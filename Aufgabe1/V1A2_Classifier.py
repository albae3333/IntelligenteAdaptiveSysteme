...
class Classifier:
    ...
    def fit(self, X, T):
        ...
        assert shapeX[0] == shapeT[
            0], "Classifier.fit(self,X,T): Data matrix X and class labels T must have same length!"
    ...
    def crossvalidate(self, S, X, T):  # do a S-fold cross validation
        ...
        for idxTest in idxS:  # loop over all possible test data sets
            ...
            X_learn, T_learn = [Xp[i] for i in idxLearn], [Tp[i] for i in
                                                           idxLearn]  # learning data for training the classifier
        ...
        for i in range(C):
            for j in range(C):
                pConfErrors[i, j] = float(pConfErrors[i, j]) / float(
                    nC[j])  # finally compute confusion error probabilities
...
if __name__ == '__main__':
    ...
    X = np.array([[1, 2, 3], [2, 3, 4], [3, 4, 5],
                  [4, 5, 6]])  # data matrix X: list of data vectors (=database) of dimension D=3
